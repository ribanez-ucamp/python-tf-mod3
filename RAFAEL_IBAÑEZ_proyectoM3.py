...
def main():
    ...
    def distribuir_balines(años):
        for h in range((años) ** 2 * 100):  # Distribuye el total de los
            ...
            for j in range(años):  # con la variable años.
                stored += randint(
                    0, 1
                )  # Esta función lo que hace es añadir un 1 o un 0
            recipientes[
                stored
            ] += 1  # con una probabilidad de ½. Esto representa lo que hacen las bolas al tropezar con uno de los pivotes,
    ...
    print(
        "\033[0;33m"
        + "Bienvenidos al simulador de la Máquina de Galton !!"
        + "\033[0;m"
    )
    print("")
    ...
    while control is False:
        while True:
            ...
            if isNumeric(s):
                ...
                if años >= 4 and años <= 20:
                    recipientes = [0] * (
                        años
                    )  # Primera posición de recipientes por años
                    break
                else:
                    print(
                        "\033[;36m"
                        + "El nivel no puede ser menor a 4 ni mayor a 20."
                        + "\033[0;m"
                    )
                    continue
            else:
        ...
        print(
            (años) ** 2 * 100, "balines usados en total"
        )  # Se despliegan los resultados, previo a graficar
        print(
            "\033[;35m"
            + "Distribución de los balines en los recipientes: "
            + "\033[0;m"
        )
        print(f"\033[;36m" + str(recipientes) + "\033[0;m")
        input("\033[;32m" + "Oprima una tecla para continuar." + "\033[0;m")
        X = np.arange(
            -((len(recipientes) / 2) - 0.5), (len(recipientes) / 2) + 0.5
        )  # Función en numpy, que devuelve valores uniformemente espaciados dentro de un intervalo dado
        fig, ax = plt.subplots()
        ...
        while continuar == " ":
            continuar = input(
                "¿Deseas ver la distribución y grafica de otros años (S/N)?"
            )
